...
int main(int argc, char **argv) {
    ...
    while (buffer[0] != 'x') {
      ...
      char askTimeMessage[] = "how many times to repeat.";
      write(client_fd, askTimeMessage, sizeof(askTimeMessage) - 1);
      // write(client_fd, buffer, strlen(buffer));
      len = read(client_fd, times_buffer, sizeof(times_buffer) - 1);
      times_buffer[len] = '\0';
      printf("-------------------\n");
      printf("%s\n", times_buffer);
      int times = atoi(times_buffer);
