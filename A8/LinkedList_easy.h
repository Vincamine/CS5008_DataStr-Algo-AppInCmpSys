...
typedef struct ll_node {
    HTKeyValuePtr  payload;  // Pointer to a string, most likely on the heap
    struct ll_node *next;     // next node in list, or NULL
    struct ll_node *prev;     // prev node in list, or NULL
} LinkedListNode, *LinkedListNodePtr;
...
// declaratioLinkedListNoden in LinkedList.h, but not the definition, similar to what we did
...
int InsertLinkedList(LinkedList, HTKeyValuePtr);
