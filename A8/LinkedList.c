//replace this file with a previous implementation of LinkedList
...
LinkedListNode* CreateLinkedListNode(HTKeyValue *data){
    ...
    if(nodeP == NULL){return NULL;}
        ...
        // nodeP->payload=data;
        nodeP->payload = (HTKeyValue*)malloc(sizeof(HTKeyValue));
        if (nodeP->payload == NULL) {
            free(nodeP);
            return NULL;
        }

        nodeP->payload->key = data->key;
        nodeP->payload->value = data->value;
        /*
...
int DestroyLinkedListNode(LinkedListNode *node){
    ...
    //     printf("ck2\n");
    //     // if(node->payload->value != NULL){
    //     //     printf("ck3\n");
    //     //     printf("%p | %s\n", node->payload->value, node->payload->value);
...
int DestroyLinkedList(LinkedList list){
    ...
    while(nodeP != NULL){
        ...
        // printf("toRemove: %p\n", toRemove);
        // printf("%p\n", nodeP);
        // printf("%u %s\n", nodeP->payload->key, nodeP->payload->value);
        // printf("%p\n", nodeP->next);
        nodeP = nodeP->next;
        if (DestroyLinkedListNode(toRemove) == 1) {
            return 1;
...
int InsertLinkedList(LinkedList list, HTKeyValuePtr data){
...
int PopLinkedList(LinkedList list, HTKeyValuePtr* dataPtr){
    if (list == NULL || list->head == NULL || dataPtr == NULL) { return 1; }
    
    // *dataPtr = (char*)malloc(sizeof(char)*(strlen(list->head->payload)+1));
    *dataPtr = (HTKeyValuePtr)malloc(sizeof(HTKeyValue));

    if (*dataPtr == NULL) {
        return 1;
    }

    (*dataPtr)->key = list->head->payload->key;
    (*dataPtr)->value = list->head->payload->value;
...
int LLIterGetPayload(LLIter iter, HTKeyValuePtr* payload) {
    ...
    *payload = (HTKeyValuePtr)malloc(sizeof(HTKeyValue));
    ...
    (*payload)->key = iter->cur_node->payload->key;
    (*payload)->value = iter->cur_node->payload->value;
...
int LLIterInsertBefore(LLIter iter, HTKeyValuePtr payload) {
